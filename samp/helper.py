...
def add_urdf_to_scene(
    urdf_path: str,
    urdf_filename: str,
    my_world: World,
    base_link_name: str = "base_link",
    subroot: str = "",
    robot_name: str = "target_gripper",
    position: np.array = np.array([0, 0, 0]),
    color: np.array = None
):
    urdf_interface = _urdf.acquire_urdf_interface()
    ...
    asset_path = get_assets_path() # curobo/src/curobo/content/assets
    full_path = join_path(asset_path, urdf_path)

    imported_urdf = urdf_interface.parse_urdf(full_path, urdf_filename, import_config)
    dest_path = subroot
    full_path = urdf_interface.import_robot(
        full_path,
        urdf_filename,
        imported_urdf,
        import_config,
    ...
    urdf_p = Robot(
        prim_path=full_path + "/" + base_link_name,
        name=robot_name,
    )

    urdf_prim = urdf_p.prim
    stage = urdf_prim.GetStage()
    linkp = stage.GetPrimAtPath(full_path)
    set_prim_transform(linkp, [position[0], position[1], position[2], 1, 0, 0, 0])

    urdf = my_world.scene.add(urdf_p)
    ...
    if color is not None:
        ...
        mtl_path = Sdf.Path(f"/World/Looks/{robot_name}_mat")
        shader = UsdShade.Shader.Define(stage, mtl_path.AppendPath("Shader"))
        shader.CreateIdAttr("UsdPreviewSurface")
        shader.CreateInput("diffuseColor", Sdf.ValueTypeNames.Color3f).Set(tuple(float(x) for x in color))
        ...
        bind_material_recursive(prim)

    return urdf, full_path
