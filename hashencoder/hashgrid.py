...
from torch.cuda.amp import custom_bwd, custom_fwd # torch 2.2.0
# from torch.amp import custom_bwd, custom_fwd # torch 2.6.0 

from .backend import _backend

class _hash_encode(Function):
    @staticmethod
    # @custom_fwd(cast_inputs=torch.half, device_type='cuda') # torch 2.6.0
    @custom_fwd(cast_inputs=torch.half) # torch 2.2.0
    ...
    @staticmethod
    # @custom_bwd(device_type='cuda') torch 2.6.0
    @custom_bwd # torch 2.2.0
...
class HashEncoder(nn.Module):
    def __init__(self, input_dim=3, num_levels=16, level_dim=2, per_level_scale=2, base_resolution=16, log2_hashmap_size=19, desired_resolution=2048):
        super().__init__()
